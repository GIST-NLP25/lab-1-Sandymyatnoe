...
def vectorize(onehot_data):
    tensor = torch.from_numpy(onehot_data).float().to(device)  # Ensure tensor is float32
    vectorized = tensor.view(tensor.size(0), -1)
    return vectorized
...
def categorize_output(data_list, data_dict):
    ...
    output_tensor = torch.tensor(output_cat).long().to(device)  # Ensure tensor is long for labels
...
class NeuralNet(nn.Module):
    def __init__(self):
        ...
        self.layer1 = LinearLayer(50920, 1000)
        self.layer2 = LinearLayer(1000, 100)
        self.layer3 = LinearLayer(100, 19)
...
def main():
    ...
    # Prepare and save the final DataFrame with predictions
    final_df = making_df(pred_test_labels)
    final_df.to_csv(f'{STUDENT_ID}_simple_seq.p1.answer.csv', index=False)
